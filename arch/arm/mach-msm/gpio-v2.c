...
#if 1 //henry for test
int gpio_tlmm_config_read(unsigned usGpio, unsigned *pusGetVaule)
{
	if (usGpio > NR_MSM_GPIOS)
		return -EINVAL;

	*pusGetVaule = readl(GPIO_CONFIG(usGpio));
	//#define GPIO_CONFIG(gpio)         (MSM_TLMM_BASE + 0x1000 + (0x10 * (gpio)))
...
EXPORT_SYMBOL(gpio_tlmm_config_read);
#endif


void cci_gpio_dump_setting(unsigned gpio)
{
	uint32_t settings, dir;
	uint32_t gpio_oe, drv_strn, gpio_func, gpio_pull,in,out;
	char out_pull[10];
	char out_drv[10];
	char out_out[5];
	char out_in[5];
	settings = readl(IOMEM(0xFA004000) + 0x1000 + (0x10 * (gpio)));
	gpio_pull = settings&0x3;
	gpio_func = (settings&0x3c)>>2;
	drv_strn = (settings&0x1c0)>>6;
	gpio_oe = (settings&0x200)>>9;
	dir = readl(IOMEM(0xFA004000) + 0x1004 + (0x10 * (gpio)));	
	out = (dir&0x2)>>1;
	in = dir&0x1;
	switch (gpio_pull)
	{
		case 0:
			sprintf(out_pull, "No Pull");
			break;
		case 1:
			sprintf(out_pull, "Pull Down");
			break;
		case 2:
			sprintf(out_pull, "Keeper");
			break;
		case 3:
			sprintf(out_pull, "Pull High");
			break;
		default:
			sprintf(out_pull, "unknown");
	}
	switch (drv_strn)
	{
		case 0:
		...
		case 3:
			...
			break;
		case 4:
			sprintf(out_drv, "10mA");
			break;
		case 5:
			sprintf(out_drv, "12mA");
			break;
		case 6:
			sprintf(out_drv, "14mA");
			break;
		case 7:
			sprintf(out_drv, "16mA");
			break;
		default:
			sprintf(out_drv, "unknown");
	}
	switch (out){
		case 0:
			sprintf(out_out, "Low");
	...
	if (gpio_oe)  //output
	{
		printk(KERN_INFO "GPIO %d Output %s read %s funcion%d drv %s %s\n", gpio, out_out, out_in, gpio_func, out_drv, out_pull);
	}
	else  //input
	{
		printk(KERN_INFO "GPIO %d Input %s function%d drv %s %s\n", gpio, out_in, gpio_func, out_drv, out_pull);
	}
	//printk(KERN_INFO "GPIO %03d %04x:%04x\n", gpio, settings, dir);
}
EXPORT_SYMBOL(cci_gpio_dump_setting);

void cci_gpio_dump_all_settings(void)
{
	int max = 172;
	int i;
	for (i=0;i<=max;i++){
		cci_gpio_dump_setting(i);
	}
}
EXPORT_SYMBOL(cci_gpio_dump_all_settings);
