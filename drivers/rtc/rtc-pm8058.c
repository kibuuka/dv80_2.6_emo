...
pm8058_rtc0_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	//B 2011/06/10 JiaHan_Li, Bug 248 - Power on reason is RTC alarm
	if (!alarm->enabled) {
	...
	//E 2011/06/10 JiaHan_Li, Bug 248 - Power on reason is RTC alarm
...
static int __devinit pm8058_rtc_probe(struct platform_device *pdev)
{
	...
	struct pm8058_chip *pm_chip;
	struct rtc_time init_date;//2011/09/02 JiaHan_Li, Bug 793 - [DA80][BSP][RTC]Set a default date
    ...
    //B 2011/09/02 JiaHan_Li, Bug 793 - [DA80][BSP][RTC]Set a default date
	pm8058_rtc0_read_time(&pdev->dev, &init_date);
	if (init_date.tm_year == 70 && init_date.tm_mon == 0 && init_date.tm_mday == 1) {
		init_date.tm_year = 111;
		init_date.tm_mon = 0;
		init_date.tm_mday = 1;
		init_date.tm_hour = 0;
		init_date.tm_min = 0;
		init_date.tm_sec = 0;
		pm8058_rtc0_set_time(&pdev->dev, &init_date);
	}
    //E 2011/09/02 JiaHan_Li, Bug 793 - [DA80][BSP][RTC]Set a default date
