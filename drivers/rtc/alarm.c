...
// modified for battery buf guard time: START
#if defined(ORG_VER)
#else
int get_rtc_timestamp(struct rtc_time* rtc_ptr)
{
	int err = 0;
	
	err = rtc_read_time(alarm_rtc_dev, rtc_ptr);
	return err;
}
EXPORT_SYMBOL(get_rtc_timestamp);
#endif
// modified for battery buf guard time: END
