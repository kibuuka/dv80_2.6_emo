					...
					u16 data_reg, int read_rows)
{
    ...
    int ii = 0;
	rc = pmic8058_kp_read(kp, new_data, data_reg, read_rows);   //read_rows==5

	for( ii  = 0 ; ii < read_rows ; ii++){
	    Printhh("[%s] new_data[%d] = %#x\n", __FUNCTION__, ii, new_data[ii]);
	}
	//
	// if press vol-up, new_data[0] = 0xbf(1011_1111), and new_data[1-4] also = 0xbf TBD??
	// if release vol-up, new_data[0] = 0xff(1111_1111), and new_data[1-4] also = 0xff TBD??
	//
					 ...
					 u16 *old_state)
{
	...
	if (kp->flags & KEYF_FIX_LAST_ROW &&
		...
		read_rows = kp->pdata->num_rows;    //read_rows=5

	if (pm8058_rev(kp->pm_chip) > PM_8058_REV_1p0)  //pm8058_rev(kp->pm_chip)==0xE3
					 ...
					 u16 *old_state)
{
	...
	for (row = 0; row < kp->pdata->num_rows; row++) {
#else
	for (row = 0; row < 1; row++) { // suppose we use GPIO_09 be drive line
#endif
		...
		for (col = 0; col < kp->pdata->num_cols; col++) {
			...
			input_sync(kp->input);
                    ...
                    //
			// henry: TBD!!, it will send 5 times key event, because row numbers is 5. Maybe need to fix it.
			//
		}
	}
...
static int __devinit pmic8058_kp_probe(struct platform_device *pdev)
{
	...
	kp->input = input_allocate_device();    // KEY_P!
	if (!kp->input) {
	...
	matrix_keypad_build_keymap(keymap_data, PM8058_ROW_SHIFT,
       ...
       #if 1   //henry add for AT command testing
       //Henry_lin, 20110526, [BugID 153] Implement testing AT command for keypad.
       //Henry_lin, 20110531, [BugID 153] Implement testing AT command for keypad.
	for(ii = 1 ; ii < 255 ; ii++)
		set_bit(ii, kp->input->keybit);
       //Henry_lin, 20110704, [BugID 386]  Implement testing AT(CMEC) command for keypad.
	set_bit(KEY_CONN_EN_CMD, kp->input->keybit);
       //Henry_lin
       #endif
...
#ifdef CONFIG_TOUCHSCREEN_ATMELMXT224_I2C
        printk("pmic8058-keypad: atmel tekpdev\n");
    	tskpdev = kp->input;
#endif
...
//Henry_lin, 20110627, [BugID 357] Change GPIO for volume up/down.
	...
	ctrl_val |= KEYP_CTRL_KEYP_EN;
             rc = pmic8058_kp_write_u8(kp, ctrl_val, KEYP_CTRL); //Henry: If mask it, irq will not run!!
