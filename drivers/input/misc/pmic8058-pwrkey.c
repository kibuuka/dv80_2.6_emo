...
extern int g_iIrqCanWakeSys[MAX_WAKE_KEY_NUM];  // define in file Msm8x60-virkey.c (drivers\input\misc)
//#endif
//Henry_lin

//Decken_Kang, 20111130, [BugID 1489] When suspending, the mute key can¡¦t work normally.
extern unsigned int g_mutekeyIrq;
...
static irqreturn_t pwrkey_press_irq(int irq, void *_pwrkey)
{
	struct pmic8058_pwrkey *pwrkey = _pwrkey;
        //2011/02/09 Jiahan_Li [BSP][Keypad]Press power key would end a call.
...
static int pmic8058_pwrkey_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		//Decken_Kang, 20111130, [BugID 1489] When suspending, the mute key can¡¦t work normally.
		enable_irq_wake(g_mutekeyIrq);

            //Henry_lin, 20110504, [BugID 66] Add wake key Camera_focus, Camera_cap, and Home key to wake up system.
            //#ifdef CONFIG_CCI_PRODUCT_DA80
            for( ii = 0 ; ii<MAX_WAKE_KEY_NUM ; ii++)
            {
                if(g_iIrqCanWakeSys[ii] != -1)
                    enable_irq_wake(g_iIrqCanWakeSys[ii]);
                else
                    break;
            }
            //#endif
...
static int pmic8058_pwrkey_resume(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		disable_irq_wake(g_mutekeyIrq);
            ...
            {
                if(g_iIrqCanWakeSys[ii] != -1)
		        disable_irq_wake(g_iIrqCanWakeSys[ii]);
		   else
		        break;
            }
            //#endif
...
#if 0    //henry mask: Qualcomm 3134 add, check later
