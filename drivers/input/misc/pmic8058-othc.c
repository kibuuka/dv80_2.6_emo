...
// Chihyu add for headset switch wrong detection, enable l5A for headset, 20120109, MILBAIDUMR-144, MILBAIDUMR-152, Bug-1761
	if (dd->othc_ir_state) {
		...
		if (rc)
			printk("othc micbais power off failed\n");
	}
...
printk("[%s][micbias_select = %d]\n",__func__,dd->othc_pdata->micbias_select);
	if (dd->othc_pdata->micbias_capability == OTHC_MICBIAS_HSED) {
