...
#include <linux/proc_fs.h>  //Robert, 20110822, QCT8x60_CR688 : Support unique USB Serial Number and #18296# for default
...
#if 0  //B: Robert, 20110822, QCT8x60_CR688 : Support unique USB Serial Number and #18296# for default
#define MAX_STR_LEN		16
/* string IDs are assigned dynamically */
#else
#define MAX_STR_LEN		17
/* string IDs are assigned dynamically */
#endif //E: Robert, 20110822, QCT8x60_CR688
...
static int get_product_id(struct android_dev *dev)
{
    ...
    if(android_config_driver.functions.next == android_config_driver.functions.prev)
    {
        printk("//get_product_id: no functions added, use default pid[0x%x]\n", dev->product_id);
        ...
        unsigned long count, void *data)
{
    ...
    if ( buf[0] == '1' ) {
        ...
        memcpy(serial_number, "1234567890ABCDEF", 16);
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
        ...
        d_entry = create_proc_entry("usb_serial", S_IRUGO | S_IWUSR | S_IWGRP | S_IROTH | S_IWOTH, NULL);
        if ( d_entry ) {
