...
#include <linux/proc_fs.h>  //Robert, 20111221, QCT8x60_CR1650: Disable USB diag port function
...
//B: Robert, 20111221, QCT8x60_CR1650 : Disable USB diag port function
static bool can_diag_work = true;
static int usb_diagctl_write_proc(struct file *file, const char __user *buffer,
        unsigned long count, void *data)
{
    ...
    if ( buf[0] == '1' ) {
        printk(KERN_INFO "Keep USB diag function.\n");
        can_diag_work = true;
    }
    else {
        printk(KERN_INFO "Disable USB diag function.\n");
        can_diag_work = false;
    }
    return count;
}

static int usb_diagctl_write2_proc(struct file *file, const char __user *buffer,
        unsigned long count, void *data)
{
    char buf[16] = {0x0};

    if ( copy_from_user(buf, buffer, 15) ) {
        printk("%s : EFAULT\n",__func__);
        return -EFAULT;
    }

    if(memcmp(buf,"OOO-OOO", 7) == 0 ) {
        printk(KERN_INFO "enable USB diag function.\n");
        can_diag_work = true;
    }
    
    return count;
}
//E: Robert, 20111221, QCT8x60_CR1650
...
static int __init diag_probe(struct platform_device *pdev)
{
	...
	//B: Robert, 20111221, QCT8x60_CR1650 : Disable USB diag port function
	{
		...
		d_entry = create_proc_entry("usb_diag_pass", S_IWUSR | S_IWGRP, NULL);
		if ( d_entry ) {
			d_entry->read_proc = NULL;
			d_entry->write_proc = usb_diagctl_write_proc;
			d_entry->data = NULL;
		}

		d_entry = create_proc_entry("usb_service_tool", S_IWUSR | S_IWGRP | S_IWOTH, NULL);
		if ( d_entry ) {
			d_entry->read_proc = NULL;
			d_entry->write_proc = usb_diagctl_write2_proc;
			d_entry->data = NULL;
		}
	}
	//E: Robert, 20111221, QCT8x60_CR1650
