...
static bool bConnectedToUSBPort = false; //Robert, 20111213, QCT8x60_CR1593: Fix fast plug in->out->in USB can't be detected
...
static int usb_get_max_power(struct usb_info *ui)
{
	...
	spin_unlock_irqrestore(&ui->lock, flags);
	bConnectedToUSBPort = false; //Robert, 20111213, QCT8x60_CR1593: Fix fast plug in->out->in USB can't be detected
    ...
    //B: Robert, 20111210, QCT8x60_CR1572 : Fix charging from self-powered hub without connecting upsteam port
    if( (temp == USB_CHG_TYPE__SDP)&&(!suspended) ) {
        if( bmaxpow==0 ) bmaxpow = 500;
        printk("usb_get_max_power: patch for unconfigured Hub bmaxpow[%d]\n", bmaxpow);
		bConnectedToUSBPort = true; //Robert, 20111213, QCT8x60_CR1593: Fix fast plug in->out->in USB can't be detected
        return bmaxpow;
    }
    if( (temp == USB_CHG_TYPE__SDP)&&(suspended)&&(!configured) ) {
        if( bmaxpow==0 ) bmaxpow = 500;
        printk("usb_get_max_power: patch for nonstandard AC bmaxpow[%d]\n", bmaxpow);
        return bmaxpow;
    }
    //E: Robert, 20111210, QCT8x60_CR1572
...
//B: Robert, 20111213, QCT8x60_CR1593 : Fix fast plug in->out->in USB can't be detected
static bool connected_to_usb = false;
bool is_connected_to_usb(void)
{
	printk("is_connected_to_usb() return %s\n", connected_to_usb ? "true":"false");
	return connected_to_usb;
}
EXPORT_SYMBOL(is_connected_to_usb);

void set_connecting_to_other_charger(void)
{
	printk("//set_connecting_to_other_charger\n");
	connected_to_usb = false;
}
EXPORT_SYMBOL(set_connecting_to_other_charger);
//E: Robert, 20111213, QCT8x60_CR1593
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	//B: Robert, 20111213, QCT8x60_CR1593 : Fix fast plug in->out->in USB can't be detected
	if( (maxpower == 500) && (bConnectedToUSBPort) )
...
static void handle_setup(struct usb_info *ui)
{
	...
	//B: Robert, 20111012, QCT8x60_CR1031 : Merge BSP3136 USB related patch
	 ...
	 */
	udelay(10);
	//E: Robert, 20111012, QCT8x60_CR1031
