...
static int wifi_probe(struct platform_device *pdev)
{
	...
	printk("## %s\n", __FUNCTION__);
	wifi_irqres = platform_get_resource_byname(pdev, IORESOURCE_IRQ, "wlan_bcm4329_irq");
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_SCAN, "dhd_wake_lock_scan_process"); //Bruno, 20111103, [DA80][BugID 1231], Add wake lock for scan process
#ifdef CONFIG_HAS_EARLYSUSPEND
...
#if 0 //Set dhdp->pktfilter[0] in dhd_preinit_ioctls()
...
dhd_detach(dhd_pub_t *dhdp)
{
	...
	/* && defined(DHD_GPL) */
		...
		WAKE_LOCK_DESTROY(dhdp, WAKE_LOCK_SCAN); //Bruno, 20111103, [DA80][BugID 1231], Add wake lock for scan process
		free_netdev(ifp->net);
		MFREE(dhd->pub.osh, ifp, sizeof(*ifp));
...
dhd_module_init(void)
{
	...
	DHD_ERROR(("%s: dhd_bus_register done\n", __FUNCTION__));
