...
DHD_ERROR (("%s: CONFIG_BCM4329_FW_PATH=%s\n", __FUNCTION__, CONFIG_BCM4329_FW_PATH));
	strncpy(fw_path, CONFIG_BCM4329_FW_PATH, MOD_PARAM_PATHLEN-1);
#else
	//fw_path[0] = '\0';
	strncpy(fw_path, "/etc/rtecdc_ap.bin\0", MOD_PARAM_PATHLEN-1);
#endif
#ifdef CONFIG_BCM4329_NVRAM_PATH
DHD_ERROR (("%s: CONFIG_BCM4329_NVRAM_PATH=%s\n", __FUNCTION__, CONFIG_BCM4329_NVRAM_PATH));
	strncpy(nv_path, CONFIG_BCM4329_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
#else
	//nv_path[0] = '\0';
	strncpy(nv_path, "/etc/Nvram.txt\0", MOD_PARAM_PATHLEN-1);
#endif
...
static char pktfilter_tmp0[256]; //Bruno
#endif
...
//B: Bruno, 20110714, [DA80][BugID 465], Wi-Fi OOB interrupt issue on suspend mode
	...
	sprintf( pktfilter_tmp0, "100 0 0 0 0xffffffffffff 0x%02x%02x%02x%02x%02x%02x",
		dhd->mac.octet[0], dhd->mac.octet[1], dhd->mac.octet[2],
		dhd->mac.octet[3], dhd->mac.octet[4], dhd->mac.octet[5]
	);
	dhd->pktfilter[0] = pktfilter_tmp0;
	DHD_ERROR(("%s: pktfilter[0] = %s\n", __FUNCTION__, dhd->pktfilter[0]));
//E: Bruno, 20110714, [DA80][BugID 465], Wi-Fi OOB interrupt issue on suspend mode
#if 0
	{
		int i;
