...
DHD_ERROR(("%s: dhdsdio_probe_init - before\n", __FUNCTION__));
...
DHD_ERROR(("%s: dhd_bus_start - before\n", __FUNCTION__));
...
DHD_ERROR(("%s: dhd_net_attach - before\n", __FUNCTION__));
	/* Ok, have the per-port tell the stack we're open for business */
	if (dhd_net_attach(bus->dhd, 0) != 0) {
		DHD_ERROR(("%s: Net attach failed!!\n", __FUNCTION__));
		goto fail;
	}
DHD_ERROR(("%s: dhd_net_attach - after\n", __FUNCTION__));
...
// for modify MAC
bool IsCorrectMACAddr(uint8* pMACAddr)
{
    int bIsCorrect = TRUE;
    if(
	    (pMACAddr[0] == 0)
...
int wlan_mac_from_file(uint8 *pCmdBuffer)
{
    void *fp;
    int wlen;
    uint8 mac_temp[12];
    int r;
    int CmdBufferCounter;

    DHD_TRACE(("%s: WLAN MAC from file \n", __FUNCTION__));
    DHD_ERROR(("%s: Original MAC = %02X:%02X:%02X:%02X:%02X:%02X \n", __FUNCTION__, pCmdBuffer[0],pCmdBuffer[1],pCmdBuffer[2],pCmdBuffer[3],pCmdBuffer[4],pCmdBuffer[5]));
    fp = dhd_os_open_image("/data/local/tmp/wmac.txt");
    if (fp == NULL)
    {
	DHD_ERROR(("%s: WLAN open: /data/local/tmp/wmac.txt fail\n", __FUNCTION__));
	return FALSE;
    }
    DHD_ERROR(("%s: WLAN MAC file opened: /data/local/tmp/wmac.txt\n", __FUNCTION__));
    wlen = dhd_os_get_image_block((void *)mac_temp, 12, fp);
    DHD_ERROR(("%s: get MAC = %c%c:%c%c:%c%c:%c%c:%c%c:%c%c \n", __FUNCTION__, mac_temp[0],mac_temp[1],mac_temp[2],mac_temp[3],mac_temp[4],mac_temp[5],mac_temp[6],mac_temp[7],mac_temp[8],mac_temp[9],mac_temp[10],mac_temp[11]));

    CmdBufferCounter = 0;

    for (r=0; r<12; r++)
    {
	if ((mac_temp[r]>=48) && (mac_temp[r]<=57)) //0-9
	{
	    if ((mac_temp[r+1]>=48) && (mac_temp[r+1]<=57)) //0-9
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-48)*16) + (mac_temp[r+1]-48));
	    else if ((mac_temp[r+1]>=65) && (mac_temp[r+1]<=70)) //A-F
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-48)*16) + (mac_temp[r+1]-55));
	    else if ((mac_temp[r+1]>=97) && (mac_temp[r+1]<=102)) //a-f
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-48)*16) + (mac_temp[r+1]-87));
	    else
	    {
		DHD_ERROR(("%s: no match pCmdBuffer[%d] = %02X \n", __FUNCTION__, CmdBufferCounter, pCmdBuffer[CmdBufferCounter]));
		return FALSE;
	    }

	    DHD_TRACE(("%s: 1.pCmdBuffer[%d] = %02X \n", __FUNCTION__, CmdBufferCounter, pCmdBuffer[CmdBufferCounter]));
	}
	else if ((mac_temp[r]>=65) && (mac_temp[r]<=70)) //A-F
	{
	    if ((mac_temp[r+1]>=48) && (mac_temp[r+1]<=57))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-55)*16) + (mac_temp[r+1]-48));
	    else if ((mac_temp[r+1]>=65) && (mac_temp[r+1]<=70))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-55)*16) + (mac_temp[r+1]-55));
	    else if ((mac_temp[r+1]>=97) && (mac_temp[r+1]<=102))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-55)*16) + (mac_temp[r+1]-87));
	    else
	    {
		DHD_ERROR(("%s: no match pCmdBuffer[%d] = %02X \n", __FUNCTION__, CmdBufferCounter, pCmdBuffer[CmdBufferCounter]));
		return FALSE;
	    ...
	    DHD_ERROR(("%s: 1.pCmdBuffer[%d] = %02X \n", __FUNCTION__, CmdBufferCounter, pCmdBuffer[CmdBufferCounter]));
	}
	else if ((mac_temp[r]>=97) && (mac_temp[r]<=102)) //a-f
	{
	    if ((mac_temp[r+1]>=48) && (mac_temp[r+1]<=57))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-87)*16) + (mac_temp[r+1]-48));
	    else if ((mac_temp[r+1]>=65) && (mac_temp[r+1]<=70))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-87)*16) + (mac_temp[r+1]-55));
	    else if ((mac_temp[r+1]>=97) && (mac_temp[r+1]<=102))
		pCmdBuffer[CmdBufferCounter] = (uint8)(((mac_temp[r]-87)*16) + (mac_temp[r+1]-87));
	    else
	    {
		DHD_ERROR(("%s: no match pCmdBuffer[%d] = %02X \n", __FUNCTION__, CmdBufferCounter, pCmdBuffer[CmdBufferCounter]));
		return FALSE;
    ...
    DHD_ERROR(("%s: WLAN MAC reading done, close file\n", __FUNCTION__));

    DHD_ERROR(("%s: %02X:%02X:%02X:%02X:%02X:%02X \n", __FUNCTION__, pCmdBuffer[0],pCmdBuffer[1],pCmdBuffer[2],pCmdBuffer[3],pCmdBuffer[4],pCmdBuffer[5]));

    if (IsCorrectMACAddr(pCmdBuffer) == FALSE)
	wlen = FALSE;
...
{
    srandom32(random32());
    return (random32()%10)+48;
}

int ModifyMACAddr(char * buf, int len, uint8* pBuffer, int rand_mac)
{
    int iStrIndex = 0;
    int i;
    char zSTR[81];
	...
	{
	    zSTR[iStrIndex] = '\0';
	    DHD_TRACE(("%s: Config file %s\n", __FUNCTION__, zSTR));
	    
	    if (strcmp(zSTR, "macaddr") == 0)
	    {
		DHD_TRACE(("%s: modify macaddr\n", __FUNCTION__));

		if (rand_mac == 0)
		{
		    //iMACLine = len;
		    sprintf(zbuffer,"%02X", pBuffer[0]);
		    buf[i+1] = zbuffer[0];
		    buf[i+2] = zbuffer[1];
		    sprintf(zbuffer,"%02X", pBuffer[1]);
		    // skip ':'3
		    buf[i+4] = zbuffer[0];
		    buf[i+5] = zbuffer[1];
		    sprintf(zbuffer,"%02X", pBuffer[2]);
		    // skip ':'6
		    buf[i+7] = zbuffer[0];
		    buf[i+8] = zbuffer[1];
		    sprintf(zbuffer,"%02X", pBuffer[3]);
		    // skip ':'9
		    buf[i+10] = zbuffer[0];
		    buf[i+11] = zbuffer[1];
		    sprintf(zbuffer,"%02X", pBuffer[4]);
		    // skip ':'12
		    buf[i+13] = zbuffer[0];
		    buf[i+14] = zbuffer[1];
		    sprintf(zbuffer,"%02X", pBuffer[5]);
		    // skip ':'15
		    buf[i+16] = zbuffer[0];
		    buf[i+17] = zbuffer[1];
		}
...
//This is for modify random MAC end
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	//for modify MAC
	uint8 octet[6]={ 0x00, 0xB0, 0x19, 0x00, 0x00, 0x00 };
	bool bIsMACAddrStatus = TRUE;
	int rand_mac = 0;
	//for modify MAC end
	...
	//for modify MAC
	bIsMACAddrStatus = wlan_mac_from_file((uint8*)(&octet[0]));
	if(bIsMACAddrStatus == FALSE)
	{
	    DHD_ERROR(("%s: WLAN MAC error, using random MAC address \n", __FUNCTION__));
	    rand_mac = 1;
	}
	//for modify MAC end
	...
	//temporal random  MAC address
	ModifyMACAddr(memblock, len, &octet[0], rand_mac);
	//temporal random  MAC address end
