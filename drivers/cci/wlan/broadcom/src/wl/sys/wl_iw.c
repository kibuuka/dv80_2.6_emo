...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		//B: Bruno, 20111006, [DA80][BugID 1010], Wi-Fi can not receive disconnect event
		if (error) {
			WL_ERROR(("%s: Fails %d\n", __FUNCTION__, error));
			return error;
		}
		//E: Bruno, 20111006, [DA80][BugID 1010], Wi-Fi can not receive disconnect event
...
//B: Bruno, 20110620, [DA80][BugID240], Implement Wi-Fi interface for T2 
static int
wl_iw_get_rssi_iwconfig(
	struct net_device *dev,
	struct iw_request_info *info,
...
)
{
	...
	WL_TRACE(("wl_iw_get_rssi_iwconfig RSSI=%d\n", rssi));

	vwrq->value = rssi;

	return error;
}
//E: Bruno, 20110620, [DA80][BugID240], Implement Wi-Fi interface for T2 
...
wl_iw_combined_scan_set(struct net_device *dev, wlc_ssid_t* ssids_local, int nssid, int nchan)
{
	...
	int i;
	iscan_info_t *iscan = g_iscan;
	wl_iw_t *iw; //Bruno, 20111103, [DA80][BugID 1231], Add wake lock for scan process

	WL_TRACE(("%s nssid=%d nchan=%d\n", __FUNCTION__, nssid, nchan));

	iw = *(wl_iw_t **)netdev_priv(dev); //Bruno, 20111103, [DA80][BugID 1231], Add wake lock for scan process
	...
	WAKE_LOCK_TIMEOUT(iw->pub, WAKE_LOCK_SCAN, 3 * HZ); //Bruno, 20111103, [DA80][BugID 1231], Add wake lock for scan process
...
static const iw_handler wl_iw_handler[] =
{
	...
	(iw_handler) NULL,			
	(iw_handler) wl_iw_get_rssi_iwconfig, //B: Bruno, 20110620, [DA80][BugID240], Implement Wi-Fi interface for T2
	(iw_handler) NULL,			
