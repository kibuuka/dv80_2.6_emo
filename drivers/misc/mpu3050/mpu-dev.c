...
#include <delay.h>
#include <err.h>
#include "../../../arch/arm/mach-msm/include/mach/cci_hw_id.h"//Check the HW version--Taylor--20111118

//huapu_cheng: start /20110829 /bugID: 739
#define PM8058_GPIO_BASE	173
#define GYRO_INT_GPIO        16 //gpio 17 on pmic8058
#define PM8058_GPIO_PM_TO_SYS(pm_gpio)	(pm_gpio + PM8058_GPIO_BASE) //-->16+173=189
//huapu_cheng: end
#define GYRO_PWR_EN_133 133

#define GPIO_AK8975_INT 40 //on msm8260
#define GPIO_PM8058_RESET 39 //gpio 40 on pm8058
#define PM8058_GPIO_BASE	173
#define PM8058_GPIO_PM_TO_SYS(pm_gpio)	(pm_gpio + PM8058_GPIO_BASE)
#define GPIO_PM8058_AK8975_RESET PM8058_GPIO_PM_TO_SYS(GPIO_PM8058_RESET)
static struct pm8058_gpio akm_gpio_config = {
        .direction      = PM_GPIO_DIR_OUT,
        .output_buffer  = PM_GPIO_OUT_BUF_CMOS,
        .output_value   = 0,  /*0->turn on, 1->turn off*/
        .pull           = PM_GPIO_PULL_NO,
        .vin_sel        = PM_GPIO_VIN_L3,
        .out_strength   = PM_GPIO_STRENGTH_LOW,
        .function       = PM_GPIO_FUNC_NORMAL,
	.inv_int_pol    = 0,
};

#define PM8058_L5_USING 1

//#if PM8058_L5_USING
static struct regulator *pm8058_l5;
//#endif
static struct regulator *pm8058_l10;


static void mpu_ak8975_gpio_init(void)
{
	int rc = -EINVAL;;
	
	printk("[compass][mpu_ak8975]>>>%s\n", __func__);
        // Bug 901, Luke GPIO tuning
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_AK8975_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN/*GPIO_CFG_NO_PULL*/,GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc)
		printk("[compass][mpu_ak8975]>>>%s>>>gpio_tlmm_config fail!!!\n", __func__);
	else
		printk("[compass][mpu_ak8975]>>>%s>>>gpio_tlmm_config ok!!!\n", __func__);

	/*reset pin setting*/
	//motoko's works
	rc = pm8058_gpio_config(GPIO_PM8058_RESET, &akm_gpio_config);
	if (rc)
       	printk("[compass][mpu_ak8975]>>>%s>>>pm8058_gpio_config(GPIO_PM8058_RESET) fail!!!\n", __func__);
	else
		printk("[compass][mpu_ak8975]>>>%s>>>pm8058_gpio_config(GPIO_PM8058_RESET) ok!!!\n", __func__);

	//gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(GPIO_PM8058_RESET), 1);
}

static void mpu3050_gpio_init(void)
{
	int rc = -EINVAL;

	/*get regulator */
//#if PM8058_L5_USING
    if (cci_hw_id < PVT1){ //Check the HW version--Taylor--20111118
	pm8058_l5 = regulator_get(NULL, "8058_l5");
    	if (IS_ERR(pm8058_l5))
		printk("[gyro][mpu-dev]>>>regulator_get(8058_l5) fail\n");
    	else
		printk("[gyro][mpu-dev]>>>regulator_get(8058_l5) success\n");
    }
//#endif
	pm8058_l10 = regulator_get(NULL, "8058_l10");
    	if (IS_ERR(pm8058_l10))
		printk("[gyro][mpu-dev]>>>regulator_get(8058_l10) fail\n");
    	else
		printk("[gyro][mpu-dev]>>>regulator_get(8058_l10) success\n");
...
//#if PM8058_L5_USING
	...
	if (rc)
		printk("[gyro][mpu-dev]>>>%s>>>regulator_set_voltage(pm8058_l5) to 2850000 fail!!!\n", __func__);
	else
    		printk("[gyro][mpu-dev]>>>%s>>>regulator_set_voltage(pm8058_l5) to 2850000 ok!!!\n", __func__);
    }
//#endif
	rc = regulator_set_voltage(pm8058_l10, 2600000, 2600000);
	if (rc)
		printk("[gyro][mpu-dev]>>>%s>>>regulator_set_voltage(pm8058_l10) to 2600000 fail!!!\n", __func__);
	else
    		printk("[gyro][mpu-dev]>>>%s>>>regulator_set_voltage(pm8058_l10) to 2600000 ok!!!\n", __func__);
...
//#if PM8058_L5_USING
	...
	if (rc)
    		printk("[gyro][mpu-dev]>>>%s>>>enable pm8058_l5 fail!!!\n", __func__);
	else
		printk("[gyro][mpu-dev]>>>%s>>>enable pm8058_l5 ok!!!\n", __func__);
	rc = regulator_disable(pm8058_l5);
	if (rc)
    		printk("[gyro][mpu-dev]>>>%s>>>disable pm8058_l5 fail!!!\n", __func__);
	else
		printk("[gyro][mpu-dev]>>>%s>>>disable pm8058_l5 ok!!!\n", __func__);
    }
//#endif
	rc = regulator_enable(pm8058_l10);
	if (rc)
    		printk("[gyro][mpu-dev]>>>%s>>>enable pm8058_l10 fail!!!\n", __func__);
	else
		printk("[gyro][mpu-dev]>>>%s>>>enable pm8058_l10 ok!!!\n", __func__);
	rc = regulator_disable(pm8058_l10);
	if (rc)
    		printk("[gyro][mpu-dev]>>>%s>>>disable pm8058_l10 fail!!!\n", __func__);
	else
		printk("[gyro][mpu-dev]>>>%s>>>disable pm8058_l10 ok!!!\n", __func__);

//huapu_cheng: start /20110829 /bugID: 739
	/* init the gyro irq*/
	rc = gpio_tlmm_config(GPIO_CFG(PM8058_GPIO_PM_TO_SYS(GYRO_INT_GPIO), 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	if(rc)
		printk("[gyro][mpu-dev]>>>gyro_gpio_irq fail\n");
//huapu_cheng: end
	/*vender suggests delay 0~20 ms between VDD and VLOGIC*/
	rc = gpio_tlmm_config(GPIO_CFG(GYRO_PWR_EN_133, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if(rc)
		printk("[gyro][mpu-dev]>>>gpio_direction_input fail\n");
	/*gpio_133 = 0 -> the current of s3 can go to gyro, hence turn on gyro*/
	gpio_set_value(GYRO_PWR_EN_133, 1);
	rc = gpio_get_value(GYRO_PWR_EN_133);
	//printk("[motoko][gyro]>>>rc>>>%d\n", rc);

       mpu_ak8975_gpio_init();
}

//[Bug 798] Luke, 2011,0902,Turn off sensors power when device suspend
char Mpu_Temp_Power_Control =0;

static void mpu3050_voltage_on(void)
{
...
//#endif
	...
	/*vender suggests to delay 0~20 ms between VDD and VLOGIC*/
	mdelay(30);
	/*gpio_133 = 0 -> the current of s3 can go to gyro, hence turn on gyro*/
	gpio_set_value(GYRO_PWR_EN_133, 0);
	rc = gpio_get_value(GYRO_PWR_EN_133);
	//printk("[motoko][gyro]>>>rc>>>%d\n", rc);
	
        //[Bug 798] Luke, 2011,0902,Turn off sensors power when device suspend 
	Mpu_Temp_Power_Control =1;
}

//disable voltage_off function by vendor's suggestion
static void mpu3050_voltage_off(void)
{
		int rc = -EINVAL;
...
/* [LA2X/LA6X] S Invensense Peter Bug#1238 remove to avoid null pointer {*/
#if 0
...
#endif
/* [LA2X/LA6X] E Invensense Peter Bug#1238 remove to avoid null pointer }*/
		  ...
		  const struct i2c_device_id *devid)
{
	...
	struct i2c_adapter *pressure_adapter = NULL;
        /* [LA2X/LA6X] S Ginger TDMS#7754 { */
	unsigned char retry;
        /* [LA2X/LA6X] E Ginger TDMS#7754 } */
...
/* [LA2X/LA6X] S Ginger TDMS#7754 { */
	...
	res = mpu3050_open(&mpu->mldl_cfg, client->adapter,
			accel_adapter, compass_adapter, pressure_adapter);
    	if(res) {
        	printk(KERN_ERR "mpu3050_open error, %s|%d, retry\n", __func__, res);
        	if (retry > 1) {
			//i2c_sensors_power_reset();
                        mpu3050_voltage_off();
            		mdelay(10);
                        mpu3050_voltage_on(); 
		        }
...
//--------------move the part irq code to here -----------------
#if 0
			if (pdata->accel.irq > 0) {
				dev_info(&this_client->adapter->dev,
					"Installing Accel irq using %d\n",
...
static int __init mpu_init(void)
{
	...
	if( cci_ftm_boot )    
	{
	    printk(KERN_INFO "[mpu3050]FTM returnl\n");
	    return -1;
	}
