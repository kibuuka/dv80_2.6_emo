...
//Henry_lin, 20110506, [BugID 76] Vibrating too long time(2.5 sec) when system boot up.
static int g_iFirstBoot = 0;
//Henry_lin
...
static int pmic8058_vib_set(struct pmic8058_vib *vib, int on)
{
	...
	if (on) {
		...
		val = vib->reg_vib_drv;
		Printhh("[%s] 111 val=%#x\n", __FUNCTION__, val);
		val |= ((vib->level << VIB_DRV_SEL_SHIFT) & VIB_DRV_SEL_MASK);  // VIB_DRV_SEL_MASK = 0xf8
		Printhh("[%s] 222 val=%#x\n", __FUNCTION__, val);
...
static void pmic8058_vib_enable(struct timed_output_dev *dev, int value)
{
	struct pmic8058_vib *vib = container_of(dev, struct pmic8058_vib,
					 timed_dev);
	unsigned long flags;
	//Decken_Kang, 20111108, [BugID 1279] Implement Qualcomm patch for Vibrator.
	//int iRet;

	Printhh("[%s] enter... value=%d\n", __FUNCTION__, value);
	//Decken_Kang, 20111108, [BugID 1279] Implement Qualcomm patch for Vibrator.
	//spin_lock_irqsave(&vib->lock, flags);
	//Henry_lin, 20110825, [BugID 729] Tap SW keyboard with vibrating will cause system reboot.
#if 0
	iRet = hrtimer_cancel(&vib->vib_timer);
//Decken_Kang, 20111108, [BugID 1279] Implement Qualcomm patch for Vibrator.
//#else
	iRet = hrtimer_try_to_cancel(&vib->vib_timer);
#endif

//Decken_Kang, 20111108, [BugID 1279] Implement Qualcomm patch for Vibrator.
#if 0
	if(iRet == -1){ //when the timer is currently excuting the callback function and cannot be stopped
	    //PrintTip("[%s] hrtimer_try_to_cancel=%d\n", __FUNCTION__, iRet);
	    mdelay(1);
...
//Decken_Kang, 20111108, [BugID 1279] Implement Qualcomm patch for Vibrator.
#if 1    //henry:Qualcomm 3134 patch, check later
retry:
	spin_lock_irqsave(&vib->lock, flags);
...
//Decken_Kang
	...
	//Henry_lin
	schedule_work(&vib->work);  //==pmic8058_vib_update()
}
...
static void pmic8058_vib_update(struct work_struct *work)
{
	...
	Printhh("[%s] enter... vib->state=%d\n", __FUNCTION__, vib->state);
	pmic8058_vib_set(vib, vib->state);
}
...

{
	...
	rc = pm_runtime_set_active(&pdev->dev); //TBD!!!!!
	if (rc < 0)
		dev_dbg(&pdev->dev, "unable to set runtime pm state\n");
	pm_runtime_enable(&pdev->dev);  //TBD!!!!!
	...
	INIT_WORK(&vib->work, pmic8058_vib_update); //TBD!!
    ...
    #if 0//Henry_lin, 20110607, [BugID 208] Provide vibrating function in aboot. So not need in driver
