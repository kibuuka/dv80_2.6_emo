...
#endif
//B: Robert, 20110516, QCT8x60_CR106 : Sync KB62 and C6 SD related patch
static unsigned long msmsdcc_irqtime = 0;
//static struct wake_lock sdcc_wakelock;

#define CARD_REINSERT_DURATION (3 * HZ) //Robert, 20111207, QCT8x60_CR1542: Fix unexpectedly remove SD card issue
#define CARD_STANDBY_INSERT_DURATION (10)
//E: Robert, 20110516, QCT8x60_CR106
...
#if 0 //B: Robert, 20111012, QCT8x60_CR1030 : Merge BSP3136 SD related patch
...
#endif //E: Robert, 20111012, QCT8x60_CR1030
	do {
		unsigned long flags;
...
#if 0  //B: Robert, 20110516, QCT8x60_CR106 : Sync KB62 and C6 SD related patch
static void
...
msmsdcc_check_status(unsigned long data)
{
	...
	if (!host->plat->status) {
		...
		host->eject = !status;
		if (status ^ host->oldstat) {
			if (!strcmp(mmc_hostname(host->mmc), "mmc1")) //external sdcard
			{
				printk(KERN_INFO
				...
				mmc_detect_change(host->mmc, duration); //duration
...
#endif  //E: Robert, 20110516, QCT8x60_CR106
		...
		const char *buf, size_t count)
{
	...
	if (host->pdev_id != 4) //Bruno, 20110920, [DA80][BugID 914], Wi-Fi OOB interrupt issue on suspend mode
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	struct msmsdcc_host *host = mmc_priv(mmc);
	int rc = 0;
	unsigned long flags;//Bruno, 20110921, [DA80][BugID921], Wi-Fi power consumption issue in suspend mode
	...
	if (mmc) {
		 ...
		 */
		pm_runtime_get_noresume(dev);
		if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO) //Bruno, 20110907, [DA80][BugID 847], Wi-Fi suspend and resume issues
		{
			rc = mmc_suspend_host(mmc);
		}
//B: Bruno, 20110921, [DA80][BugID921], Wi-Fi power consumption issue in suspend mode
		else
		{
...
//E: Bruno, 20110921, [DA80][BugID921], Wi-Fi power consumption issue in suspend mode
