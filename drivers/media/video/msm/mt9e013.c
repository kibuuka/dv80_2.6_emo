...
#include <linux/regulator/consumer.h>/*[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
#include "mt9e013.h"
#include <mach/cci_hw_id.h>//[Camera] AF power changes in EVT2 board
...
static uint16_t mt9e013_linear_total_step = 37;
static uint16_t mt9e013_step_jump = 3;
#define MT9E013_TOTAL_STEPS_NEAR_TO_FAR_MAX    37

uint16_t mt9e013_step_position_table[MT9E013_TOTAL_STEPS_NEAR_TO_FAR_MAX+1];
static uint16_t mt9e013_nl_region_boundary1 = 1;
static uint16_t mt9e013_nl_region_code_per_step1 = 62;
static uint16_t mt9e013_l_region_code_per_step = 4;
static uint16_t mt9e013_damping_threshold = 22;
static uint16_t mt9e013_sw_damping_time_wait = 30;
static uint16_t mt9e013_sw_damping_small_step = 3;
static uint16_t mt9e013_sw_damping_time_wait_coarse = 24;
static uint16_t mt9e013_sw_damping_small_step_coarse = 2;
static uint16_t mt9e013_sw_damping_time_wait_fwd = 24;
static uint16_t mt9e013_sw_damping_small_step_fwd = 4;
static uint16_t mt9e013_sw_damping_time_wait_fwd_4 = 60;
static uint16_t mt9e013_sw_damping_small_step_fwd_4 = 3;
static uint16_t mt9e013_sw_damping_time_wait_fine = 60;
static uint16_t mt9e013_sw_damping_small_step_fine = 2;
static uint16_t mt9e013_sw_damping_time_wait_reverse = 30;
static uint16_t mt9e013_sw_damping_small_step_reverse = 4;
static uint16_t mt9e013_sw_damping_time_wait_snap = 22;
static uint16_t mt9e013_sw_damping_small_step_snap = 4;
static uint16_t mt9e013_sw_damping_time_wait_fine_fwd = 30;
static uint16_t mt9e013_sw_damping_small_step_fine_fwd = 2;
static uint16_t mt9e013_enable_damping = 1;
static uint16_t mt9e013_af_initial_value = 62;
static uint16_t mt9e013_damping_threshold_1 = 14;
static uint16_t mt9e013_damping_threshold_2 = 4;
static uint16_t mt9e013_damping_threshold_3 = 6;
static uint16_t mt9e013_damping_threshold_4 = 12;
...
static struct regulator *vreg_AVDD_L15;
static struct regulator *vreg_VDD_S3;
...
static uint16_t mt9e013_get_prev_lines_pf(void)
{
	if (mt9e013_ctrl->prev_res == QTR_SIZE)
        ...
        /* B:[bug947] 1080p support, cci 20110926 */
	else if (mt9e013_ctrl->prev_res == VIDEO_SIZE)
		return mt9e013_regs.reg_video[E013_FRAME_LENGTH_LINES].wdata;
        /* E:[bug947] 1080p support, cci 20110926 */
	else
		return mt9e013_regs.reg_120fps[E013_FRAME_LENGTH_LINES].wdata;
}
...
static uint16_t mt9e013_get_prev_pixels_pl(void)
{
	if (mt9e013_ctrl->prev_res == QTR_SIZE)
		...
		return mt9e013_regs.reg_video[E013_LINE_LENGTH_PCK].wdata;
        /* E:[bug947] 1080p support, cci 20110926 */
	else
		return mt9e013_regs.reg_120fps[E013_LINE_LENGTH_PCK].wdata;
}
...
static int32_t mt9e013_set_fps(struct fps_cfg   *fps)
{
	...
	if (mt9e013_ctrl->curr_res == QTR_SIZE)
        ...
        /* B:[bug947] 1080p support, cci 20110926 */
	else if (mt9e013_ctrl->curr_res == VIDEO_SIZE)
		total_lines_per_frame =
		mt9e013_regs.reg_video[E013_FRAME_LENGTH_LINES].wdata;
        /* E:[bug947] 1080p support, cci 20110926 */
	else
		total_lines_per_frame =
		mt9e013_regs.reg_120fps[E013_FRAME_LENGTH_LINES].wdata;
	...
	int32_t num_steps)
{
	...
	int16_t move_to_lens_position;
	int16_t target_dist, small_step, next_lens_position;
	uint16_t sw_damping_time_wait = 1;
	int16_t af_nl_lens_pos =
		mt9e013_step_position_table[mt9e013_nl_region_boundary1];
	...
	CDBG("%s: Step:%d, Lens:%d\n", __func__, dest_step_position,
		dest_lens_position);
	move_to_lens_position = dest_lens_position;
	if (mt9e013_enable_damping) {
		...
		if (step_direction < 0) {
			if (num_steps >= mt9e013_damping_threshold) {
				/* sweeping towards all the way
				   in infinity direction between snaps*/
				small_step = mt9e013_sw_damping_small_step_snap;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_snap;
			} else if (num_steps >= mt9e013_damping_threshold_1) {
				/* reverse search during macro mode */
				small_step =
					mt9e013_sw_damping_small_step_reverse;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_reverse;
			} else if (num_steps >= mt9e013_damping_threshold_2) {
				/* reverse search fine mode */
				small_step = mt9e013_sw_damping_small_step;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait;
			} else {
				small_step = mt9e013_sw_damping_small_step_fine;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_fine;
			}
			if (dest_step_position < mt9e013_nl_region_boundary1) {
				if (mt9e013_ctrl->curr_step_pos <
				...
				move_to_lens_position = af_nl_lens_pos;
			}
		} else {
			if (num_steps <= mt9e013_step_jump) {
				small_step =
					mt9e013_sw_damping_small_step_coarse;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_coarse;
			} else if (num_steps <= mt9e013_damping_threshold_3) {
				/* reverse search during macro mode */
				small_step =
					mt9e013_sw_damping_small_step_fine_fwd;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_fine_fwd;
			} else if (num_steps <= mt9e013_damping_threshold_4) {
				/* reverse search during macro mode */
				small_step =
					mt9e013_sw_damping_small_step_fwd_4;
				sw_damping_time_wait =
					mt9e013_sw_damping_time_wait_fwd_4;
			} else {
			...
			if (mt9e013_ctrl->curr_step_pos <
				...
				mt9e013_i2c_write_w_sensor(REG_VCM_NEW_CODE,
					move_to_lens_position);
				usleep(sw_damping_time_wait*50);
				mt9e013_ctrl->curr_lens_pos = af_nl_lens_pos;
				move_to_lens_position = dest_lens_position;
			}
		}
		...
		for (next_lens_position = mt9e013_ctrl->curr_lens_pos
			...
			(step_direction * move_to_lens_position);
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	printk("[Camera]-%s, sensor_settings, type(%d), rt(%d)\n",__func__, update_type,rt);
	...
	if (update_type == REG_INIT) {
		...
		if (rt == QTR_SIZE) {
			...
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_snap,
                ...
                /* B:[bug947] 1080p support, cci 20110926 */
		} else if (rt == VIDEO_SIZE) {
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_pll,
				mt9e013_regs.reg_pll_size);
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_video,
				mt9e013_regs.reg_video_size);
                /* E:[bug947] 1080p support, cci 20110926 */
...
/*B:[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
            #ifdef CONFIG_ARCH_MSM8X60//Cloony
                    msm_camio_clk_rate_set(192000000);
            #else
...
/*B:[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
static int32_t mt9e013_power_up(void)
{
    CDBG("[Camera]-%s: ++\n", __func__);
    ...
    vreg_VDD_S3 = regulator_get(NULL, "8058_s3");
    if (IS_ERR(vreg_VDD_S3)) {
        pr_err("%s: VREG 8058_s3 get failed\n", __func__);
        vreg_VDD_S3 = NULL;
        return 0;
    }
    if (regulator_set_voltage(vreg_VDD_S3, 1800000, 1800000)) {
        pr_err("%s: VREG 8058_s3 set voltage failed\n",  __func__);
        goto s3_disable;
    }
    if (regulator_enable(vreg_VDD_S3)) {
        pr_err("%s: VREG 8058_s3 enable failed\n", __func__);
        goto s3_put;
    }
    
    vreg_AVDD_L15 = regulator_get(NULL, "8058_l15");
    if (IS_ERR(vreg_AVDD_L15)) {
        pr_err("%s: VREG LDO15 get failed\n", __func__);
        vreg_AVDD_L15 = NULL;
        return 0;
    }
    if (regulator_set_voltage(vreg_AVDD_L15, 2800000, 2800000)) {
        pr_err("%s: VREG LDO15 set voltage failed\n",  __func__);
        goto ldo15_disable;
    }
    if (regulator_enable(vreg_AVDD_L15)) {
        pr_err("%s: VREG LDO15 enable failed\n", __func__);
        goto ldo15_put;
    }

    /*B:[Camera] AF power changes in EVT2 board, CCI 20110801 */
    if(cci_hw_id >= EVT2)
    {
        gpio_set_value(1, 1);    //Turn on AF power(GPIO_1)

    }
    /*E:[Camera] AF power changes in EVT2 board, CCI 20110801 */
    
    return 0;

ldo15_disable:
	regulator_disable(vreg_AVDD_L15);
ldo15_put:
	regulator_put(vreg_AVDD_L15);
s3_disable:
	regulator_disable(vreg_VDD_S3);
s3_put:
	regulator_put(vreg_VDD_S3);
    return 0;
...
static int32_t mt9e013_power_down(void)
{
    ...
    if(cci_hw_id >= EVT2)
    {
        gpio_set_value(1, 0);    //Turn off AF power(GPIO_1)
...
/*B:[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
    ...
    if(cci_hw_id >= EVT2)
    {
        gpio_tlmm_config(GPIO_CFG(1,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    }
    /*E:[Camera] AF power changes in EVT2 board, CCI 20110801 */
    // power up
    mt9e013_power_up();
/*E:[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
    ...
    s->s_mount_angle  = 90;/*[Camera] DA80 porting MT9M114/MT9E013, CCI 20110408 */
...
static int mt9e013_set_af_stepparam(void *data, u64 val)
{
	...
	CDBG("%s: step:%d, coarse:%d, fine:%d, snap:%d, fwd:%d\n", __func__,
		mt9e013_sw_damping_small_step,
		mt9e013_sw_damping_small_step_coarse,
		mt9e013_sw_damping_small_step_fine,
		mt9e013_sw_damping_small_step_snap,
...
static int mt9e013_set_af_timeparam(void *data, u64 val)
{
	...
	CDBG("%s: time:%d, coarse:%d, fine:%d, snap:%d, fwd:%d\n", __func__,
		mt9e013_sw_damping_time_wait,
		mt9e013_sw_damping_time_wait_coarse,
		mt9e013_sw_damping_time_wait_fine,
		mt9e013_sw_damping_time_wait_snap,
...
static int mt9e013_set_af_initial(void *data, u64 val)
{
	...
	CDBG("af_initial:%d, step:%d, wait:%d\n", mt9e013_af_initial_value,
		mt9e013_sw_damping_small_step_fwd,
		mt9e013_sw_damping_time_wait_fwd);
	return 0;
}
...
static int mt9e013_set_threshold(void *data, u64 val)
{
	mt9e013_damping_threshold = val & 0xFF;
	mt9e013_damping_threshold_1 = (val >> 8) & 0xFF;
	mt9e013_damping_threshold_2 = (val >> 16) & 0xFF;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(af_threshold, NULL,
			mt9e013_set_threshold, "%lld\n");
...
static int cam_debug_init(void)
{
	...
	if (!debugfs_create_file("af_timeparam", S_IRUGO | S_IWUSR, cam_dir,
							 NULL, &cam_timeparam))
		return -ENOMEM;
	if (!debugfs_create_file("af_threshold", S_IRUGO | S_IWUSR, cam_dir,
							 NULL, &af_threshold))
		return -ENOMEM;
