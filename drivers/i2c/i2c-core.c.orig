...
// added by yafi-begin
	...
	if (addr==0x35)
	{
		if (command != 0x5)
		{
//	PrintLog_DEBUG("%s adapter->algo->smbus_xfer = 0x%x\n", __func__, (unsigned int)(adapter->algo->smbus_xfer));
//	PrintLog_DEBUG("%s addr = 0x%x\n", __func__, addr);
//	PrintLog_DEBUG("%s flags = 0x%x\n", __func__, flags);
	PrintLog_DEBUG("read_write = %s\n", ((read_write == I2C_SMBUS_WRITE)?"Write":"Read"));
	PrintLog_DEBUG("command = 0x%x\n", command);
//	PrintLog_DEBUG("%s protocol = 0x%x\n", __func__, protocol);
	if (read_write == I2C_SMBUS_WRITE)
		PrintLog_DEBUG("byte = 0x%x\n", data->byte);
		}
	}
#endif
// added by yafi-end
