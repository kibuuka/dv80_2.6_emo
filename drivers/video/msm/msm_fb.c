...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	var->activate = FB_ACTIVATE_VBL,	/* activate it at vsync */
        //CCI Luke change panel size -->
	var->height = 95, // -1	/* height of picture in mm */
	var->width = 53,//-1,	/* width of picture in mm */
        //CCI Luke change panel size <--
...
int cci_fb_UpdateDone = 0;//CH: fix snow picture issue
static int msm_fb_pan_display(struct fb_var_screeninfo *var,
			      struct fb_info *info)
{
	...
	if( cci_fb_UpdateDone == 0 )
	{
		printk("[LCM]::%s: cci_fb_UpdateDone !!!! \n", __func__);
		cci_fb_UpdateDone = 1;
		msm_fb_set_backlight(mfd, mfd->panel_info.bl_min);
	}
