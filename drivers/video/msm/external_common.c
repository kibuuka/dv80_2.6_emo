...
//B: CH if no edid 720p output
#if 0
		external_common_state->video_resolution;
#else
		HDMI_VFRMT_1280x720p60_16_9;
        printk("hdmi::%s: edid read fail, set 720p output \n", __func__);
#endif
//E: CH if no edid 720p output
	return status;
}
...
bool hdmi_common_get_video_format_from_drv_data(struct msm_fb_data_type *mfd)
{
	...
	if (var->reserved[3]) {
		...
		case 1440:
			...
			break;
		case 1920:
			format = HDMI_VFRMT_1920x1080p24_16_9;//CH fix MHL discolor issue
			break;
		}
	}
