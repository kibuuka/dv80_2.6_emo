...
//S:[CCI FTM], Leon, for CCI FTM via Apps 
#ifndef ORG_VER
/*CmdID for APP Handling*/
//S:[CCI FTM], StanleyC, RTC command  removed to modem
#define DIAG_ODM_SUBCMD_ADCRead		0x10
#define DIAG_ODM_CMD_RESET_CFC 	0x51	//DIAG_ODM_SUBCMD_RESET_CFC, Leon add for prevent modem dead if reset in modem side
...
#define DIAG_ODM_SUBCMD_GET_RTC 0x26
#define DIAG_ODM_SUBCMD_SET_RTC 0x2f
...
static unsigned char FTM_APP_CMD[] = 
{
	...
	DIAG_ODM_CMD_RESET_CFC, //DIAG_ODM_SUBCMD_RESET_CFC, Leon add for prevent modem dead if reset in modem side
//E:[CCI FTM], StanleyC, RTC command  removed to modem
	DIAG_ODM_SUBCMD_BACKUP_STATUS,
	DIAG_ODM_SUBCMD_CHG_SWITCH,
	DIAG_ODM_SUBCMD_SIMPLE_TEST,
	DIAG_ODM_SUBCMD_GET_RTC,
	DIAG_ODM_SUBCMD_SET_RTC,
};
#endif
//E:[CCI FTM], Leon, for CCI FTM via Apps 
...
static int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	temp += 2;
//S:[CCI FTM], Leon modify for dispatching CCI FTM command to android and modem side		
#ifndef ORG_VER
#ifdef FTM_DEBUG_KERNEL		
				printk(KERN_INFO "diag kernel log: buf[0]~[3] = 0x%X, 0x%X, 0x%X, 0x%X", buf[0], buf[1], buf[2], buf[3]);
#endif
#endif
//E:[CCI FTM], Leon modify for dispatching CCI FTM command to android and modem side
		...
		if (driver->table[i].process_id != 0) {
			if (driver->table[i].cmd_code ==
				...
				if (driver->table[i].cmd_code_lo <=
						...
						//S:[CCI FTM], Leon, for CCI FTM and SimpleTest via Apps 
						#ifndef ORG_VER 
						if(cmd_code == 238)  //parse packets start of 0xEE ...
						{
							//S: leon add for check if this pkt should pass to android FTM
							int k = 0;
							...
							{
								#ifdef FTM_DEBUG_KERNEL									
								printk(KERN_INFO "diag_process_apps_pkt .. FTM_APP_CMD[%d]= %2x\n",k,(int)FTM_APP_CMD[k]);
								#endif
								if (cmdID == (int)FTM_APP_CMD[k])
								{
									...
									#ifdef FTM_DEBUG_KERNEL											
									printk(KERN_INFO "diag_process_apps_pkt .. CCI FTM cmdID[%d] for APP handling\n",FTM_APP_CMD[k]);
									#endif
									return 0;
								}
							}
							//E: leon add for check if this pkt should pass to android FTM
						}
						else
						{
						#endif				
						//E:[CCI FTM], Leon, for CCI FTM and SimpleTest via Apps 
					driver->pkt_length = len;
					diag_update_pkt_buffer(buf);
					diag_update_sleeping_process
						(driver->table[i].
