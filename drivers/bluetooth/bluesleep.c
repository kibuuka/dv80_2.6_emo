...
{
	if (bluesleep_hdev != NULL) {
        BT_DBG("bluesleep_hdev != NULL");
        if (on) {
...
{
	if (bluesleep_hdev != NULL &&
                (test_bit(BT_ASLEEP, &flags) ||(!gpio_get_value(bsi->ext_wake)) )) {
		BT_DBG("waking up...");
...
{
	char proto;
    int iId = 0, iCurrent = 40;    // 0 means BT
    //int iRet = 0;
...
#if 1   //henry test for current measure.
    //Printhh("[%s] proto = %c\n", __FUNCTION__, proto);
    //iRet = system_loading_update(LOADING_GET, iId, 0); 
    //Printhh("[%s] bef iRet = %d\n", __FUNCTION__, iRet);
    if (proto == '0'){
        //disable BT
        Printhh("[%s] disbale BT\n", __FUNCTION__);
        system_loading_update(LOADING_CLR, iId, 0);
    }
    else{
        //enable BT
        Printhh("[%s] enable BT\n", __FUNCTION__);
        system_loading_update(LOADING_CLR, iId, 0);
        system_loading_update(LOADING_SET, iId, iCurrent);
    }
    //iRet = system_loading_update(LOADING_GET, iId, 0); 
    //Printhh("[%s] aft iRet = %d\n", __FUNCTION__, iRet);
 #endif
